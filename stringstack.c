...
static struct StkNode *
GetTopStkNode(struct Stack *Stk)
{
	if (!Stk)
		return NULL;

	if (Stk->StringList.mlh_TailPred ==
		(struct MinNode *)&Stk->StringList.mlh_Head)
...
char *
PushString(struct Stack *Stk, char *Str)
{
	if (!Stk || !Str)
		return NULL;
...
PopString(struct Stack *Stk)
{
	...
	StkNode =
		(struct StkNode *)RemTail((struct List *)&Stk->StringList);
	if (!StkNode)
		return NULL;
	...
	if (Stk->PopStringBuf && Stk->PopStringBufSize >= (Len + 1)) {
		...
		Stk->PopStringBuf =
			(char *)MALLOCFUNC(Stk->PopStringBufSize = (Len + 1));
		if (!Stk->PopStringBuf) {
			FREEFUNC(StkNode, StkNode->AllocLen);
			return NULL;
		}
...
int
AddChar(struct Stack *Stk, int Chr)
{
	...
	ExtraSpace =
		StkNode->AllocLen - sizeof (struct StkNode) - Len - 1 - 1;
	if (ExtraSpace < 0) {
		SpaceAlloc =
			((((-ExtraSpace * STRINCREMENT) / STRINCREMENT) +
			  STRINCREMENT)) + StkNode->AllocLen;
		Pred = StkNode->Node.mln_Pred;
		Remove((struct Node *)StkNode);
		StkNode = (struct StkNode *)realloc(OldNode =
											StkNode, SpaceAlloc);
		if (!StkNode) {			/* if error, put it back on list and return error */
			Insert((struct List *)&Stk->StringList,
				   (struct Node *)OldNode, (struct Node *)Pred);
			return -1;
		}
		Insert((struct List *)&Stk->StringList,
			   (struct Node *)StkNode, (struct Node *)Pred);
...
char *
AddStr(struct Stack *Stk, char *Str)
{
	struct StkNode *StkNode, *OldNode;
	struct MinNode *Pred;
	int Len, ExtraSpace, SpaceAlloc;
	...
	ExtraSpace =
		StkNode->AllocLen - sizeof (struct StkNode) - Len - 1 -
		strlen(Str);
	if (ExtraSpace < 0) {
		SpaceAlloc =
			((((-ExtraSpace * STRINCREMENT) / STRINCREMENT) +
...
InitString(struct Stack *Stk)
{
	...
	StkNode =
		(struct StkNode *)CALLOCFUNC(sizeof (struct StkNode) +
									 STRINCREMENT);
	if (!StkNode)
		return NULL;
...
int
CountStack(struct Stack *Stk)
	 /* Count the number of entries on a stack */
{
	struct MinNode *Node;
	int Cnt = 0;

	for (Node = Stk->StringList.mlh_Head; Node->mln_Succ;
		 Node = Node->mln_Succ)
		Cnt++;
	return Cnt;
}
