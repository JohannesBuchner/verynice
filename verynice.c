...
char *versionstring =
	"VeryNice version " VERSION " "
	"Copyright (C) 1992-2000 Stephen D. Holland.\n"
	"This program is distributed under the terms of the GNU General Public License,\n"
	"version 2.\n";

#define min_mac(a,b) (((a) < (b)) ? (a) : (b))

//#define PREFIX "/usr/local" /* PREFIX should be defined on compiler command line */
...
double
timediff(struct timeval *starttime, struct timeval *endtime)
{
	int deltasec;
	int deltausec;
...
void
DeleteProc(struct procent *proc)
{
	free(proc->exename);
	free(proc);
}
...
void
procinsert(struct List *list, struct procent *proc)
	 /* insert into sorted list */
{
	struct procent *node;

	for (node = (struct procent *)list->lh_Head; node;
		 node = (struct procent *)node->Node.ln_Succ) {
...
struct procent *
procfindbypid(struct List *plist, pid_t pid)
{
	struct procent *proc;

	for (proc = (struct procent *)plist->lh_Head; proc->Node.ln_Succ;
	 ...
	 /* search for a UID in the uidlist */
{
	...
	for (k = (struct knownuid *)uidlist.lh_Head; k->Node.ln_Succ;
		 k = (struct knownuid *)k->Node.ln_Succ) {
		if (k->uid == uid) {
			return k;
		}
	}
...
ReadProcs(struct List *Target)
{
	...
	while (ent = readdir(dir)) {
		...
		if (!ent->d_name[Pos]) {
			...
			if (statf) {
				actsize =
					fread((char *)statfbuf, 1,
						  sizeof (statfbuf) - 1, statf);
				assert(actsize >= 0);
				...
				for (lastcloseparenidx = actsize - 1;
					 lastcloseparenidx >= 0; lastcloseparenidx--) {
				...
				if (lastcloseparenidx > 0) {
					...
					exelname =
						malloc(6 + strlen(ent->d_name) + 1 + 3 + 1);
...
updatecpuusage(void)
{
	...
	for (proc = (struct procent *)proclist.lh_Head,
		...
		if ((!nproc->Node.ln_Succ
			 || (proc->Node.ln_Succ && (nproc->pid > proc->pid)))) {
			/* nproc is missing -- process dead -- do nothing */
			/* increment proc */
			proc = (struct procent *)proc->Node.ln_Succ;

		}
		else if ((!proc->Node.ln_Succ
				  || (nproc->Node.ln_Succ
					  && (nproc->pid < proc->pid)))) {
...
int
MatchString(char *str, char *substr)
{
	int LenDiff;
	int Cnt;
...
void
SetProcFlags(struct procent *proc)
{
	struct badexe *b;
	...
	for (g = (struct goodexe *)gexelist.lh_Head; g->Node.ln_Succ;
		 g = (struct goodexe *)g->Node.ln_Succ) {
		if (g->alluid) {
	...
	for (iuid = (struct immuneuid *)iuidlist.lh_Head;
		 iuid->Node.ln_Succ;
		 iuid = (struct immuneuid *)iuid->Node.ln_Succ) {
	...
	for (b = (struct badexe *)bexelist.lh_Head; b->Node.ln_Succ;
		 b = (struct badexe *)b->Node.ln_Succ) {
	...
	for (h = (struct hungryexe *)hungrylist.lh_Head; h->Node.ln_Succ;
		 h = (struct hungryexe *)h->Node.ln_Succ) {
	...
	for (r = (struct runawayexe *)runawaylist.lh_Head;
		 r->Node.ln_Succ; r = (struct runawayexe *)r->Node.ln_Succ) {
	...
	for (i = (struct immuneexe *)iexelist.lh_Head; i->Node.ln_Succ;
		 i = (struct immuneexe *)i->Node.ln_Succ) {
...
MergeProcs(void)
{
	...
	do {
		didsomething = 0;
		for (nproc = (struct procent *)newproclist.lh_Head;
			 nproc->Node.ln_Succ; nproc = next) {
			next = (struct procent *)nproc->Node.ln_Succ;
			if (nproc->reallynew) {
				...
				if (proc) {
					...
					if (nproc->uid == proc->uid) {
						...
						nproc->potentialrunaway =
							proc->potentialrunaway;
					}
...
ReniceProcs(double deltat)
{
	...
	if (idletime > deltat)
		idletime = deltat;		/* we're interested in maximum
								 * idle time on a SINGLE processor */

	if (idletime < 0) {
		if ((-idletime) > ((double)periodicity) / 100.0) {
			syslog(LOG_WARNING,
				   "%g seconds of CPU used in %g seconds on %d processors\n",
	...
	if (idletime > largest)
		largest = idletime;		/* idle counts as a process, in this case */
	...
	for (proc = (struct procent *)proclist.lh_Head;
		...
		if (!proc->immuneflag && !proc->goodflag) {
			...
			badkarmaincreasefactor =
				1 / (1 +
					 4 * min_mac(proc->badkarma,
								 batchjob) / batchjob);

			if (proc->hungryflag)	/* assume CPU hungry process */
				badkarmaincrease =
					deltat * badkarmarate * badkarmaincreasefactor;
			else
				badkarmaincrease =
					proc->current_cpuusage * deltat * karmarate *
					badkarmaincreasefactor -
					deltat * karmarecoverrate * (largest -
												 proc->
												 current_cpuusage);

			if (badkarmaincrease > 1.0)
				badkarmaincrease = 1.0;	/* upper bound rate of
		...
		if (!proc->immuneflag && proc->badflag
		...
		if (proc->nicelevel > 0 && proc->nicelevel - proc->reniced > 0
		...
		if (desirednicelevel != proc->nicelevel
			&& (!proc->immuneflag || proc->reniced)) {
			/* renice process */
			if (desirednicelevel < -20)
				desirednicelevel = -20;
			if (desirednicelevel > 20)
				desirednicelevel = 20;
			ret =
				setpriority(PRIO_PROCESS, proc->pid,
							desirednicelevel);
			if (!ret) {
				/* setpriority worked! */
		...
		if (proc->potentialrunaway && proc->badkarma > killproc) {
			syslog(LOG_WARNING,
				   "Sending SIGKILL to process %ld, executable %s, uid %ld. BadKarma=%g",
				   (long)proc->pid,
				   (char *)((proc->exename == NULL) ? "" :
							proc->exename), (long)proc->uid,
				   (double)proc->badkarma);
			kill(proc->pid, SIGKILL);	/* die!!! */
		}
		else if (proc->potentialrunaway && proc->badkarma > runaway) {
			syslog(LOG_WARNING,
				   "Sending SIGTERM to process %ld, executable %s, uid %ld. BadKarma=%g",
				   (long)proc->pid,
				   (char *)((proc->exename == NULL) ? "" :
							proc->exename), (long)proc->uid,
				   (double)proc->badkarma);
...
UnniceProcs(void)
{
	...
	while (proc = (struct procent *)RemHead(&proclist)) {
		...
		if (targetlevel > 20)
			targetlevel = 20;
		if (!proc->immuneflag)
			setpriority(PRIO_PROCESS, proc->pid, targetlevel);
...
performdump(void)
{
	...
	for (proc = (struct procent *)proclist.lh_Head;
		 ...
		 proc = (struct procent *)proc->Node.ln_Succ) {
		syslog(LOG_INFO,
			   "%s: pid %ld, nice %ld+%ld, parent %ld, at %g secs CPU (current %g), badkarma %g, uid %ld %s %s %s %s %s",
			   proc->exename, (long)proc->pid,
			   (long)(proc->nicelevel - proc->reniced),
			   (long)proc->reniced, (long)proc->parentpid,
			   (double)proc->cpuusage,
			   (double)proc->current_cpuusage,
			   (double)proc->badkarma, (long)proc->uid,
			   (proc->immuneflag == 0) ? "" : "immune",
			   (proc->goodflag == 0) ? "" : "good",
			   (proc->badflag == 0) ? "" : "bad",
			   (proc->potentialrunaway ==
				0) ? "" : "potentialrunaway",
			   (proc->hungryflag == 0) ? "" : "hungry");
...
main(int argc, char *argv[])
{
	...
	if (daemonpidfile) {
		...
		if (mypid = fork()) {
			...
			if (pidfile) {
				...
				fprintf(stderr,
						"Error writing process ID file: %s\n",
						daemonpidfile);
				exit(1);
			}
		}
	...
	if (numprocessors < 1 || numprocessors > 100)
		numprocessors = 1;
	...
	for (Cnt = 0; !killflag; Cnt++) {
		...
		if (timediff(&end, &start) > .95 * periodicity) {
			...
			if (timediff(&start, &end) >
				((double)periodicity) / 100.0) {
